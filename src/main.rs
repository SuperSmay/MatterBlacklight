...
// --- START: Backlight Control Logic Configuration ---

// **IMPORTANT:** You must replace this path with the actual path to your
// Linux backlight device. E.g., /sys/class/backlight/intel_backlight
...
// Helper function to read the max brightness
fn get_max_backlight() -> Result<u32, Error> {
    ...
    fs::read_to_string(path)
        .map_err(|e| {
            ...
            // FIX: Explicit type annotation for `into()`
...
// Helper function to map a Matter Level (0-254) to the Linux brightness range (0-Max)
fn map_level_to_brightness(level: u8, max_brightness: u32) -> u32 {
    if level == 0 {
        return 0;
    }
    let hardware_range = max_brightness as f64;
    let level_f64 = level as f64;

    // Scale Matter level (1 to 254) to the actual hardware brightness range (1 to max_brightness)
    let mapped_brightness = (level_f64 / 254.0) * hardware_range;

    mapped_brightness.round().max(1.0) as u32 // Ensure minimum brightness is at least 1 when level > 0
}

// Helper function to map a Linux brightness value (0-Max) to a Matter Level (0-254)
fn map_brightness_to_level(brightness: u32, max_brightness: u32) -> u8 {
    if brightness == 0 {
        return 0;
    }
    let matter_range = 254.0;
    let hardware_max = max_brightness as f64;
    let brightness_f64 = brightness as f64;

    let mapped_level = (brightness_f64 / hardware_max) * matter_range;

    // Ensure the result is between 1 and 254
    mapped_level.round().max(1.0).min(254.0) as u8
}

// --- END: Backlight Control Logic Configuration ---
...
impl LevelControlDeviceLogic {
    pub fn new() -> Self {
        let max_brightness = get_max_backlight().unwrap_or(255); // Fallback to 255
...
impl LevelControlHooks for LevelControlDeviceLogic {
    ...
    fn set_device_level(&self, level: u8) -> Result<Option<u8>, ()> {
        let brightness = map_level_to_brightness(level, self.max_brightness);

        info!(
            "LevelControl: Matter Level {} mapped to Backlight Brightness {}",
            level, brightness
        );

        match self.write_backlight(brightness) {
            Ok(_) => Ok(Some(level)),
            Err(e) => {
                error!("Error setting backlight brightness: {:?}", e);
                Err(())
            }
        }
    }
    ...
    fn current_level(&self) -> Option<u8> {
        match self.read_backlight() {
            Ok(brightness) => {
                let level = map_brightness_to_level(brightness, self.max_brightness);
                Some(level)
            },
            Err(e) => {
...
pub struct OnOffDeviceLogic {
    // on_off field removed, state is derived from LevelControl
...
impl OnOffDeviceLogic {
    pub fn new() -> Self {
        ...
        let persisted_state = match fs::File::open(storage_path.as_path()) {
            Ok(mut file) => {
                ...
                // We only care about start_up_on_off from persistence
    ...
    // Helper to write to backlight for on/off state
    fn set_backlight_on_off(&self, on: bool) -> Result<(), Error> {
        // Use a temporary LevelControlDeviceLogic to access the backlight write helpers and max_brightness
        let level_logic = LevelControlDeviceLogic::new();
        
        // Use 50% of the max Matter level (127/254) as a good default for 'ON'.
        let default_on_level = 127; 

        let brightness = if on {
            map_level_to_brightness(default_on_level, level_logic.max_brightness)
        } else {
            0
        };

        info!("OnOff: Setting backlight to brightness {} for state {}", brightness, on);

        level_logic.write_backlight(brightness)
    }
...
impl OnOffHooks for OnOffDeviceLogic {
    ...
    fn on_off(&self) -> bool {
        // Read the actual brightness and check if it's > 0
        let level_logic = LevelControlDeviceLogic::new();
        match level_logic.read_backlight() {
    ...
    fn set_on_off(&self, on: bool) {
        ...
        if let Err(err) = self.set_backlight_on_off(on) {
            error!("Error setting backlight for OnOff: {}", err);
        }

        // Save only start_up_on_off
